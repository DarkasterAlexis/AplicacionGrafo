...
def dibujar_grafo(p_frame, grafo, camino=None, mst=None, titulo="Grafo de Servidores"):
    ...
    nx.draw_networkx_edges(G, pos, edge_color="yellow", ax=ax)
    labels = nx.get_edge_attributes(G, "weight")
    nx.draw_networkx_edge_labels(G, pos, edge_labels=labels, ax=ax)

    if camino:
        edges_camino = [(camino[i], camino[i+1]) for i in range(len(camino)-1)]
        nx.draw_networkx_edges(G, pos, edgelist=edges_camino, edge_color="black", width=4, ax=ax)

    if mst:
        edges_mst = [(u, v) for u, v, _ in mst]
        nx.draw_networkx_edges(G, pos, edgelist=edges_mst, edge_color="maroon", width=4, ax=ax)
